...
const repairProxy = recoveryTypes.find(row => row.type === "換電/修復代理");
...
root.innerHTML = `
	...
	<main class="deck">
		...
		${slide(
			"03 / Methods",
			"統計方法如何對應課程",
			`${bullets(["描述統計：平均車數、缺車率、低庫存率、Top 10 排序。", "隨機變數：每個觀測窗是否缺車視為 0/1 事件。", "兩樣本決策：清大 vs 交大、星期一 vs 星期五，用比例 z 檢定與 95% 信賴區間。", "敏感度與限制：電量門檻、補車代理規則、期末期間外推限制。"])}`
		)}
		${slide(
			"04 / Q1",
			"每日策略：不要只看校區總數",
		...
		${slide(
			...
			"高風險不是只因為完全沒車",
			`${chart("station-risk")}
			<p>Top 10 站點多數缺車來自「有車但不可借」，不是單純車流被借走。</p>`
		)}
		${slide(
			"07 / Q3",
			"不可借原因：電量與 errorMsg 重疊",
			`${chart("reason-signals")}
			${bullets([`不可借快照中，官方 errorMsg 第一名是 ${errorMessages[0]?.message ?? "NA"}。`, "低電量與 errorMsg 不是互斥分類，不能把比例直接相加。", "Connection: close 視為 API 傳輸雜訊，不列入車輛錯誤 Top 類型。"])}`
		)}
		${slide(
			"08 / Q4",
			"35% 附近是明顯風險門檻",
			...
			<div class="metric-grid compact">
				${metric("<35% 快照不可借率", percent(lowBatteryCritical * 100, 1))}
		...
		${slide(
			...
			${bullets([`換電/修復代理約 ${percent(repairProxy?.share ?? 0)}。`, `大量回補約 ${percent(rebalance?.share ?? 0)}，比例很小。`, "站點恢復多半是總車數不變但可借狀態回來，或小量歸還。"])}`
		)}
		${slide(
			"10 / Tests",
			"檢定結果支持策略差異",
		...
		${slide(
			"11 / Strategy",
			"怎麼比較借得到",
			`${bullets(["先看附近站點，不只看校區總量。", "星期一清大避開清晨，上學前要保留走路或校車備案。", "星期五交大跨校或下坡移動建議 15:00 前完成，避免 16:00-17:00 才找車。", "看到車但不能借時，優先懷疑低電量或官方 errorMsg，而不是單純沒有車。"])}`
		)}
		${slide(
			"12 / Conclusion",
			"結論與限制",
			`${bullets(["借不到的核心不是單純缺車，而是低庫存、低電量與異常狀態同時作用。", "官方 errorMsg 最大宗是 illegalMovement；電量 <35% 也覆蓋多數不可借快照。", "資料期間跨學期末與暑假初期，且電量分析採系統性快照估計。", "後續若有更多時間，應補逐 imei 全量 join 與實際借車成功/失敗紀錄。"])}`
		)}
	</main>
`;
